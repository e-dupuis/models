...
def _get_params_from_object(model_type, dataset, model_dir, data_dir, mode, config_file, params_override,
                            run_eagerly=None, tpu=None, log_steps=100):
    """Get ParamsDict from flags."""
    ...
    overriding_configs = (config_file, params_override,
                          flags_overrides)
...
def train_and_eval(
    ...
    yaml_path = os.path.join(params.model_dir, "gpu.yaml")
    ...
    if isinstance(additional_callbacks, list):
        ...
        model.compile(
            ...
            metrics=metrics,
            steps_per_execution=steps_per_loop)
    model.evaluate(validation_dataset, verbose=1, steps=1)
